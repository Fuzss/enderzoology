...
public class EnderInfestedZombieOuterLayer<T extends Zombie> extends RenderLayer<T, ZombieModel<T>> {
    ...
    public EnderInfestedZombieOuterLayer(RenderLayerParent<T, ZombieModel<T>> pRenderer, EntityModelSet p_174491_) {
        super(pRenderer);
        this.model = new ZombieModel<>(p_174491_.bakeLayer(ModelLayers.ZOMBIE));
    }
